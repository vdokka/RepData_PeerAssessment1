...
It can be fetched from this location over the internet [here](https://d396qusza40orc.cloudfront.net/repdata%2Fdata%2Factivity.zip)
...
Using read.csv(), we have the data in a dataframe already in a format suitable for plotting. By converting the date strings to Date objects we can extract info on week days vs. weekends which will be used for rest of of the assignment.

## What is the mean total number of steps taken per day?

For this part of the assignment we are asked to EXCLUDE rows with NA (not available) values.
...
## [1] 15264
```
We see that a few thousand rows are dropped from the data set in this manner.

Using ggplot2, we create a histogram of summed numbers of steps per date taken from this  data set with out NA values:
...
As suggested in the assignment, we can interpolate the NA values by taking the mean number of steps from the 'complete' dataset we obtained earlier. We copy the original data set, loop through it to find NA data, and use the 'mean steps per interval' table as a lookup table to fill in these values.
...
By analysing the above plots, we can conclude that there are HIGH 'peaks' in activity on week days when compared to weekend days -- probably because during work hours the individual from whom the data was collected is often seated, and they walk mostly after work hours and during their commute to and from work.
